...
class Firemixgolem:
    images = None
    Firemixgolem_sound = None

    def load_images(self):
        if Firemixgolem.images == None:
            Firemixgolem.images = {}
            for name in animation_names:
                if name == 'Idle':
                    Firemixgolem.images[name] = [load_image("./Mob/Firemixgolem/"+ name + " (%d)" % i + ".png") for i in range(1, 3 + 1)]
                elif name == 'Walk':
                    Firemixgolem.images[name] = [load_image("./Mob/Firemixgolem/" + name + " (%d)" % i + ".png") for i in range(1, 4 + 1)]
                elif name == 'Hit':
                    Firemixgolem.images[name] = [load_image("./Mob/Firemixgolem/"+ name + " (1)" + ".png")]
                elif name == 'Die':
                    Firemixgolem.images[name] = [load_image("./Mob/Firemixgolem/" + name + " (%d)" % i + ".png") for i in range(1, 7 + 1)]
    ...
    def __init__(self, i=0.0, j=0):
        ...
        if Firemixgolem.Firemixgolem_sound == None:
            Firemixgolem.Firemixgolem_sound = load_wav("./Sound/Hitsound.mp3")
            Firemixgolem.Firemixgolem_sound.set_volume(8)

    def update(self):
        self.sx = self.x - server.background.window_left
        self.x = clamp(self.base_x - 180.0, self.x, self.base_x + 180.0)
    ...
    def handle_collision(self, group, other):
        if group == 'server.character:firemixgolem' and (self.state == 0 or self.state == 1):
            other.take_damage(8)
        elif group == 'normalrf:firemixgolem' and (self.state == 0 or self.state == 1 or self.state == 3 or self.state == 6):
            self.take_damage(other.give_damage())
            other.get_count()
        elif group == 'normalrfsp:firemixgolem' and (self.state == 0 or self.state == 1 or self.state == 3 or self.state == 6):
            self.take_damage(other.give_damage())
            other.get_count()
        elif group == 'normalhg:firemixgolem' and (self.state == 0 or self.state == 1 or self.state == 3 or self.state == 6):
            self.take_damage(other.give_damage())
            other.get_count()
    ...
    def take_damage(self, damage):
        if (self.state == 0 or self.state == 1 or self.state == 3 or self.state == 6) and not self.delay:
            ...
            Firemixgolem.Firemixgolem_sound.play()
            if self.hp <= 0:
                self.state = 4
                self.frame = 0
    ...
    def walk(self):
        ...
        if self.x <= self.base_x - 180.0 or self.x >= self.base_x + 180:
            self.face_dir *= -1
    ...
    def build_behavior_tree(self):
        ...
        self.bt = BehaviorTree(Action('firemixgolem_AI', run_state_actions))