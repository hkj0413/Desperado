...
class Pig:
    ...
    def draw(self):
        if -25 <= self.sx <= 1600 + 25:
            if not self.state == 5:
                if self.face_dir == 1:
                    self.images[self.name][int(self.frame)].composite_draw(0, 'h', self.sx - 3, self.y + 8, 70, 60)
                elif self.face_dir == -1:
                    self.images[self.name][int(self.frame)].composite_draw(0, '', self.sx + 3, self.y + 8, 70, 60)
                if character.God:
                    draw_rectangle(*self.get_rect())

    def get_bb(self):
        return self.x - 30.0, self.y - 17.0, self.x + 30.0, self.y + 18.0

    def get_rect(self):
        return self.sx - 30.0, self.y - 17.0, self.sx + 30.0, self.y + 18.0
