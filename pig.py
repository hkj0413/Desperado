...
class Pig:
    ...
    def load_images(self):
        if Pig.images == None:
            ...
            for name in animation_names:
                if name == 'Idle':
                    Pig.images[name] = [load_image("./Mob/Pig/"+ name + " (%d)" % i + ".png") for i in range(1, 2 + 1)]
                elif name == 'Walk':
                    Pig.images[name] = [load_image("./Mob/Pig/" + name + " (%d)" % i + ".png") for i in range(1, 3 + 1)]
                elif name == 'Hit':
                    Pig.images[name] = [load_image("./Mob/Pig/"+ name + " (1)" + ".png")]
                elif name == 'Die':
                    Pig.images[name] = [load_image("./Mob/Pig/" + name + " (%d)" % i + ".png") for i in range(1, 3 + 1)]
    ...
    def draw(self):
        if -25 <= self.sx <= 1080 + 25:
            if not self.state == 5:
                if self.face_dir == 1:
                    self.images[self.name][int(self.frame)].composite_draw(0, 'h', self.sx - 3, self.y + 10, 70, 60)
                elif self.face_dir == -1:
                    self.images[self.name][int(self.frame)].composite_draw(0, '', self.sx + 3, self.y + 10, 70, 60)
                if character.God:
                    draw_rectangle(*self.get_rect())

    def get_bb(self):
        return self.x - 30.0, self.y - 15.0, self.x + 30.0, self.y + 20.0

    def get_rect(self):
        return self.sx - 30.0, self.y - 15.0, self.sx + 30.0, self.y + 20.0

    def handle_collision(self, group, other):
        if group == 'server.character:pig' and (self.state == 0 or self.state == 1):
            other.take_damage(2)
        elif group == 'normalrf:pig' and (self.state == 0 or self.state == 1 or self.state == 3 or self.state == 6):
            self.take_damage(other.give_damage())
            other.get_count()
        elif group == 'normalrfsp:pig' and (self.state == 0 or self.state == 1 or self.state == 3 or self.state == 6):
            self.take_damage(other.give_damage())
            other.get_count()
        elif group == 'normalhg:pig' and (self.state == 0 or self.state == 1 or self.state == 3 or self.state == 6):
            self.take_damage(other.give_damage())
            other.get_count()
    ...
    def build_behavior_tree(self):
        ...
        self.bt = BehaviorTree(Action('pig_AI', run_state_actions))